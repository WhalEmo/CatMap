package com.beem.catmap.ui.profile.post

import android.app.Application
import androidx.lifecycle.AndroidViewModel
import androidx.lifecycle.viewModelScope
import com.beem.catmap.data.local.UserSession
import com.beem.catmap.data.repository.PostRepository
import com.beem.catmap.data.model.Post
import com.beem.catmap.ui.manager.CatEventBus
import com.beem.catmap.ui.manager.CatMapEvent
import com.beem.catmap.ui.manager.ProfileEvent
import com.beem.catmap.ui.manager.ProfileEventBus
import com.beem.catmap.ui.manager.UiMessageManager
import com.beem.catmap.ui.manager.UiMessageState
import com.google.firebase.firestore.DocumentSnapshot
import kotlinx.coroutines.Job
import kotlinx.coroutines.flow.MutableSharedFlow
import kotlinx.coroutines.flow.MutableStateFlow
import kotlinx.coroutines.flow.SharedFlow
import kotlinx.coroutines.flow.StateFlow
import kotlinx.coroutines.flow.asSharedFlow
import kotlinx.coroutines.flow.asStateFlow
import kotlinx.coroutines.flow.update
import kotlinx.coroutines.launch

class PostViewModel(application: Application) : AndroidViewModel(application) {

    private val repository = PostRepository.Companion.getInstance(application)

    private val _uiState = MutableStateFlow(PostUiState())
    val uiState: StateFlow<PostUiState> = _uiState.asStateFlow()

    private val _mapsDeleteEvent = MutableSharedFlow<Boolean>(replay = 0)
    val mapsDeleteEvent: SharedFlow<Boolean> = _mapsDeleteEvent.asSharedFlow()

    private val _postDetailState = MutableStateFlow<Post?>(null)
    val postDetailState: StateFlow<Post?> = _postDetailState.asStateFlow()

    private val _loaderId = MutableStateFlow("")
    val loaderId: StateFlow<String> = _loaderId.asStateFlow()

    // Coroutine Job takipleri
    private var fetchPostsJob: Job? = null
    private var loadMoreJob: Job? = null

    init {
        observeProfileEvents()
    }

    private fun observeProfileEvents() {
        viewModelScope.launch {
            ProfileEventBus.profileEvent.collect { event ->
                when (event) {
                    is ProfileEvent.PostAdded -> {
                        val state = _uiState.value
                        if (state.isLoading) return@collect
                        if (state.posts.any { it.catId == event.post.catId }) return@collect
                        _uiState.update {
                            it.copy(
                                posts = listOf(event.post) + it.posts,
                                isEmpty = false
                            )
                        }
                    }

                    is ProfileEvent.PostDeleted -> {
                        _uiState.update { state ->
                            val updatedPosts = state.posts.filterNot { it.catId == event.catId }
                            state.copy(
                                posts = updatedPosts,
                                isEmpty = updatedPosts.isEmpty()
                            )
                        }
                    }

                    else -> {}
                }
            }
        }
    }

    fun setYukleyenID(id: String) {
        _loaderId.value = id
    }

    /**
     * DÜZELTİLDİ: Profil grid UI State'i tetiklemeden sadece detayı getirir.
     */
    fun getPostDetail(kediId: String, forceRefresh: Boolean = false, onComplete: (Post?) -> Unit) {
        viewModelScope.launch {
            repository.getPostDetail(kediId, forceRefresh)
                .onSuccess { gonderi ->
                    _postDetailState.value = gonderi
                    onComplete(gonderi)
                }
                .onFailure { exception ->
                    UiMessageManager.emitMessage(
                        UiMessageState.Error(
                            exception.localizedMessage ?: "Gönderi detayları alınamadı."
                        )
                    )
                    onComplete(null)
                }
        }
    }

    fun getMorePosts() {
        val currentState = _uiState.value
        val userId = _loaderId.value

        if (userId.isBlank() ||
            currentState.isMoreLoading ||
            currentState.isLoading ||
            currentState.isLastPage ||
            currentState.lastDocument == null
        ) {
            return
        }

        loadMoreJob?.cancel()
        loadMoreJob = viewModelScope.launch {
            _uiState.update { it.copy(isMoreLoading = true) }

            repository.getUsersPost(userId = userId, lastDocument = currentState.lastDocument)
                .onSuccess { pageResult ->
                    _uiState.update { state ->
                        val updatedPosts = state.posts + pageResult.posts
                        state.copy(
                            posts = updatedPosts,
                            isEmpty = updatedPosts.isEmpty(),
                            isMoreLoading = false,
                            isLastPage = pageResult.isLastPage,
                            lastDocument = pageResult.lastDocument
                        )
                    }
                }
                .onFailure { exception ->
                    _uiState.update { it.copy(isMoreLoading = false) }
                    UiMessageManager.emitMessage(
                        UiMessageState.Error(exception.localizedMessage ?: "Daha fazla gönderi alınamadı.")
                    )
                }
        }
    }

    fun postDelete(userId: String, kediId: String) {
        viewModelScope.launch {
            _uiState.update { it.copy(isLoading = true) }

            repository.userPostDelete(userId, kediId)
                .onSuccess {
                    UiMessageManager.emitMessage(UiMessageState.Success("Gönderi başarıyla silindi."))

                    val updatedPosts = _uiState.value.posts.filterNot { it.catId == kediId }
                    _uiState.update {
                        it.copy(
                            posts = updatedPosts,
                            isEmpty = updatedPosts.isEmpty(),
                            isLoading = false
                        )
                    }

                    ProfileEventBus.emitEvent(ProfileEvent.PostDeleted(catId = kediId))
                }
                .onFailure { exception ->
                    _uiState.update { it.copy(isLoading = false) }
                    UiMessageManager.emitMessage(
                        UiMessageState.Error(exception.localizedMessage ?: "Gönderi silinemedi.")
                    )
                }
        }
    }

    fun mapAndPostDeleteCat(userId: String, catId: String) {
        if (catId.isBlank()) return
        viewModelScope.launch {
            _uiState.update { it.copy(isLoading = true) }

            val gonderiRes = if (userId.isNotBlank()) repository.userPostDelete(userId, catId) else Result.success(Unit)
            val haritaRes = repository.mapCatDelete(catId)

            if (gonderiRes.isSuccess && haritaRes.isSuccess) {
                val updatedPosts = _uiState.value.posts.filterNot { it.catId == catId }
                _uiState.update {
                    it.copy(
                        posts = updatedPosts,
                        isEmpty = updatedPosts.isEmpty(),
                        isLoading = false
                    )
                }

                CatEventBus.emitEvent(CatMapEvent.Deleted(catId = catId))
                ProfileEventBus.emitEvent(ProfileEvent.PostDeleted(catId = catId))
                _mapsDeleteEvent.emit(true)
                UiMessageManager.emitMessage(UiMessageState.Success("Haritadan silindi."))
            } else {
                _uiState.update { it.copy(isLoading = false) }
                val errorMsg = postDeleteError(gonderiRes, haritaRes)
                UiMessageManager.emitMessage(UiMessageState.Error(errorMsg))
            }
        }
    }

    fun setPostLoadingState() {
        _uiState.update {
            it.copy(
                isLoading = true,
                isAccessDenied = false,
                isEmpty = false,
                isLastPage = false,
                lastDocument = null
            )
        }
    }

    fun getPosts(
        userId: String,
        isFollowing: Boolean = true,
        forceRefresh: Boolean = false
    ) {
        if (userId.isBlank()) return
        setYukleyenID(userId)

        fetchPostsJob?.cancel()
        loadMoreJob?.cancel()

        val isSelfProfile = userId == UserSession.userId
        val canAccess = isSelfProfile || isFollowing

        if (!canAccess) {
            _uiState.update {
                it.copy(
                    posts = emptyList(),
                    isEmpty = false,
                    isLoading = false,
                    isAccessDenied = true,
                    isLastPage = true,
                    lastDocument = null
                )
            }
            return
        }

        fetchPostsJob = viewModelScope.launch {
            // DÜZELTİLDİ: Sadece ilk yüklemede veya zorunlu yenilemede posts listesini sıfırla,
            // aksi halde cache'ten hızlıca gelecek veride anlık ekran boşalması (flicker) olmasın.
            _uiState.update {
                it.copy(
                    isLoading = true,
                    isAccessDenied = false,
                    isEmpty = false,
                    posts = if (forceRefresh) emptyList() else it.posts,
                    lastDocument = null,
                    isLastPage = false
                )
            }

            repository.getUsersPost(
                userId = userId,
                lastDocument = null,
                forceRefresh = forceRefresh
            )
                .onSuccess { pageResult ->
                    _uiState.update { currentState ->
                        currentState.copy(
                            posts = pageResult.posts,
                            isEmpty = pageResult.posts.isEmpty(),
                            isLoading = false,
                            isAccessDenied = false,
                            isLastPage = pageResult.isLastPage,
                            lastDocument = pageResult.lastDocument
                        )
                    }
                }
                .onFailure { exception ->
                    _uiState.update { it.copy(isLoading = false, isAccessDenied = true) }
                    UiMessageManager.emitMessage(
                        UiMessageState.Error(exception.localizedMessage ?: "Gönderiler yüklenemedi.")
                    )
                }
        }
    }

    private fun postDeleteError(gonderiRes: Result<Unit>, haritaRes: Result<Unit>): String {
        return gonderiRes.exceptionOrNull()?.localizedMessage
            ?: haritaRes.exceptionOrNull()?.localizedMessage
            ?: "Silme işlemi sırasında hata oluştu."
    }

    fun setupFromFullProfile(
        userId: String,
        initialPosts: List<Post>,
        lastDoc: DocumentSnapshot? = null,
        isLast: Boolean = true,
        isAccessDenied: Boolean = false
    ) {
        setYukleyenID(userId)

        _uiState.update {
            it.copy(
                posts = initialPosts,
                isEmpty = initialPosts.isEmpty() && !isAccessDenied,
                isLoading = false,
                isAccessDenied = isAccessDenied,
                isLastPage = isLast,
                lastDocument = lastDoc,
            )
        }
    }

    fun setAccessDenied(isDenied: Boolean) {
        _uiState.update { currentState ->
            currentState.copy(
                isAccessDenied = isDenied,
                posts = emptyList(),
                isLoading = false,
                isEmpty = false,
                isLastPage = true,
                lastDocument = null
            )
        }
    }

    fun resetAndSetBlocked() {
        _uiState.update {
            it.copy(
                isEmpty = true,
                posts = emptyList(),
                isAccessDenied = true,
                isLoading = false
            )
        }
    }

}